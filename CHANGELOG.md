...
### Changed
- **Export refuses to emit a placeholder as a token.** `exportConfigAuthenticated()` used to substitute `***DECRYPTION_FAILED***` for a field it could not decrypt; that string passed the import-side token check (non-empty, >= 10 characters) and was stored as a real auth token, producing an API that looks configured and fails at request time. Export now refuses and names the entry, and both placeholders are rejected by token validation — an older export importing one yields an entry with no token yet, keeping its URL, model and env vars intact.
...
- **Two files are now created next to the config and never deleted automatically.** `<config>.pre-key-migration` holds the ciphertext from before a key-generation migration; `<config>*.unreadable.N` holds generations you chose to set aside, which "Restore a set-aside config" in API management can bring back if their key becomes reachable again. Keep both.
