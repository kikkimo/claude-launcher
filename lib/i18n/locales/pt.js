...
module.exports = {
    ...
    menu: {
        ...
        api_management: {
            ...
            quarantine_config: "Separar a configuração ilegível e recomeçar",
            restore_quarantined: "Restaurar uma configuração separada",
    ...
    // Message types
    warnings: {
        config_load_error: "O arquivo de configuração de API está ilegível e NÃO foi sobrescrito: {0}\nAbra o gerenciamento de APIs de terceiros para separá-lo com segurança (nada é excluído) e recomeçar.",
        config_recovered: "O arquivo de configuração da API estava corrompido — recuperado automaticamente do backup: {0}",
        superseded_preserved: "A geração substituída NÃO foi excluída — ela é mantida em {0}.",
        snapshot_available: "Um instantâneo legível de uma configuração anterior à migração ainda está no disco: {0}\nEle nunca é usado automaticamente. Veja a seção de backup do README para restaurá-lo.",
        snapshot_unreadable: "Há no disco um instantâneo anterior à migração que a chave atual não consegue abrir: {0}\nNÃO o exclua: ele pode voltar a ser legível em outra rede ou depois de restaurar o arquivo de material de chave.",
        quarantine_available: "Uma configuração separada anteriormente ainda está no disco. Tente «Restaurar uma configuração separada» no gerenciamento de APIs de terceiros.",
        token_unrecoverable: "Não foi possível descriptografar o token de autenticação destas APIs: {0}\nInsira-os novamente. O texto cifrado anterior é mantido em {1} e nada foi excluído.",
        key_material_degraded: "Material de chave da máquina: {0}",
        config_unreadable_key_material: "O material de chave da máquina está ilegível, portanto nada pode ser descriptografado: {0}\nSeu arquivo de configuração de API está muito provavelmente INTACTO — não o exclua. Repare ou remova o arquivo de material de chave; nesta máquina a identidade normalmente pode ser rederivada.\nDetalhes: {1}",
    },

    quarantine: {
        cancelled: "Cancelado — nada foi alterado.",
        confirm: "Separar {0} e começar com uma configuração vazia?\nOs arquivos são apenas RENOMEADOS (para *.unreadable.N): nada é excluído e você poderá restaurá-los depois.",
        done: "Separado como *.unreadable.{0} ({1} arquivo(s)). Nada foi excluído.",
        entry_readable: "#{0} — legível agora, {1} API(s): {2}",
        entry_unreadable: "#{0} — ainda ilegível nesta máquina",
        failed: "Não foi possível separar a configuração ({0}). Nada foi alterado.",
        restore_confirm: "Restaurar a configuração separada #{0}? A configuração em uso é mantida como geração de backup.",
        restore_done: "Configuração separada #{0} restaurada ({1} API(s)).",
        restore_failed: "Não foi possível restaurar ({0}). Nada foi alterado.",
        restore_none: "No momento, nenhuma configuração separada pode ser aberta nesta máquina.",
        restore_title: "Configurações separadas",
    },
    ...
    errors: {
        ...
        validation: {
            ...
            auth_token_placeholder: "Isso é um marcador que a exportação grava quando falta o token, não um token",
    ...
    import_export: {
        export: {
            ...
            details_skipped: "Deixadas de fora (não foi possível descriptografar o token armazenado): {0}",
    ...
    version: {
        ...
        install_command: "Execute npm install -g @kikkimo/claude-launcher@latest para atualizar",
    ...
    version_check: {
        ...
        update_command: "Comando de atualização: npm install -g @kikkimo/claude-launcher@latest",
    ...
    hints: {
        ...
        model: {
            ...
            fable: "Corresponde ao nível Fable do Claude Code",
            fable_detail: "Modelo topo de linha para tarefas de longa duração e em segundo plano. Corresponde à variável de ambiente [ANTHROPIC_DEFAULT_FABLE_MODEL]. Mapeado automaticamente para o topo de linha do provedor",
