...
module.exports = {
    ...
    menu: {
        ...
        api_management: {
            ...
            quarantine_config: "隔离无法读取的配置并重新开始",
            restore_quarantined: "恢复已隔离的配置",
            back: "返回主菜单"
        },
        config: {
            title: "配置管理",
    ...
    // 消息类型
    warnings: {
        config_load_error: "API 配置文件无法读取，未被覆盖：{0}\n可进入「第三方 API 管理」将其安全隔离（不会删除任何文件）后重新开始。",
        config_recovered: "API 配置文件已损坏——已自动从备份恢复：{0}",
        superseded_preserved: "被顶替的那一代**没有被删除**，保留在 {0}。",
        snapshot_available: "磁盘上仍有一份可读的迁移前配置快照：{0}\n它不会被自动使用。恢复步骤见 README 的备份章节。",
        snapshot_unreadable: "磁盘上有一份迁移前快照，但当前密钥打不开它：{0}\n请**不要删除**——换到别的网络、或恢复密钥材料文件之后，它可能重新变得可读。",
        quarantine_available: "磁盘上仍保留着先前隔离的配置。可在「第三方 API 管理」中选择「恢复已隔离的配置」尝试找回。",
        token_unrecoverable: "以下 API 的鉴权令牌无法解密：{0}\n请重新输入。此前的密文保留在 {1}，未删除任何内容。",
        key_material_degraded: "机器密钥材料：{0}",
        config_unreadable_key_material: "机器密钥材料无法读取，导致任何内容都解不开：{0}\n你的 API 配置文件大概率是**完好**的——请不要删除它。应当修复或移除该密钥材料文件；在这台机器上身份通常可以重新推导出来。\n详情：{1}",
    },

    quarantine: {
        cancelled: "已取消——未做任何改动。",
        confirm: "是否隔离 {0} 并以空配置重新开始？\n文件只会被重命名（改为 *.unreadable.N），不会删除任何内容，之后仍可恢复。",
        done: "已隔离为 *.unreadable.{0}（共 {1} 个文件）。未删除任何内容。",
        entry_readable: "#{0} —— 现在可读，{1} 个 API：{2}",
        entry_unreadable: "#{0} —— 在这台机器上仍无法读取",
        failed: "隔离失败（{0}）。未做任何改动。",
        restore_confirm: "是否恢复已隔离的配置 #{0}？当前正在使用的配置会保留为备份代际。",
        restore_done: "已恢复隔离配置 #{0}（{1} 个 API）。",
        restore_failed: "恢复失败（{0}）。未做任何改动。",
        restore_none: "当前这台机器上没有可以打开的已隔离配置。",
        restore_title: "已隔离的配置",
    },
    ...
    errors: {
        ...
        validation: {
            ...
            auth_token_placeholder: "这是导出时用于占位的标记，不是真实令牌",
    ...
    import_export: {
        export: {
            ...
            details_skipped: "已跳过（其保存的令牌无法解密）：{0}",
    ...
    version: {
        ...
        install_command: "运行 npm install -g @kikkimo/claude-launcher@latest 更新",
    ...
    version_check: {
        ...
        update_command: "更新命令：npm install -g @kikkimo/claude-launcher@latest",
    ...
    hints: {
        ...
        model: {
            ...
            fable: "对应 Claude Code Fable 层级",
            fable_detail: "用于长程任务与后台任务的旗舰模型。对应环境变量 [ANTHROPIC_DEFAULT_FABLE_MODEL]。自动映射到供应商旗舰模型",
    ...
    config_labels: {
        model: {
            ...
            ANTHROPIC_DEFAULT_FABLE_MODEL: "Fable 模型（长程任务）",
