...
module.exports = {
    ...
    menu: {
        ...
        api_management: {
            ...
            quarantine_config: "읽을 수 없는 설정을 격리하고 새로 시작",
            restore_quarantined: "격리된 설정 복원",
            back: "메인 메뉴로 돌아가기"
        },
        config: {
            title: "설정 관리",
    ...
    // 메시지 타입
    warnings: {
        config_load_error: "API 설정 파일을 읽을 수 없어 덮어쓰지 않았습니다: {0}\n서드파티 API 관리에서 안전하게 격리한 뒤(파일은 삭제되지 않습니다) 새로 시작할 수 있습니다.",
        config_recovered: "API 구성 파일이 손상되었습니다 — 백업에서 자동 복구되었습니다: {0}",
        superseded_preserved: "교체된 세대는 **삭제되지 않았습니다** — {0}에 보관되어 있습니다.",
        snapshot_available: "마이그레이션 이전 설정의 스냅샷이 디스크에 남아 있으며 읽을 수 있습니다: {0}\n자동으로 사용되지 않습니다. 복원 방법은 README의 백업 절을 참고하세요.",
        snapshot_unreadable: "마이그레이션 이전 스냅샷이 디스크에 있지만 현재 키로는 열 수 없습니다: {0}\n**삭제하지 마세요** — 다른 네트워크에서, 또는 키 자료 파일을 복구하면 다시 읽을 수 있게 될 수 있습니다.",
        quarantine_available: "이전에 격리한 설정이 디스크에 남아 있습니다. 서드파티 API 관리의 \"격리된 설정 복원\"을 시도해 보세요.",
        token_unrecoverable: "다음 API의 인증 토큰을 복호화하지 못했습니다: {0}\n다시 입력해 주세요. 이전 암호문은 {1}에 보관되어 있으며 삭제된 것은 없습니다.",
        key_material_degraded: "머신 키 자료: {0}",
        config_unreadable_key_material: "머신 키 자료를 읽을 수 없어 아무것도 복호화할 수 없습니다: {0}\nAPI 설정 파일은 대부분 **손상되지 않았습니다** — 삭제하지 마세요. 대신 키 자료 파일을 복구하거나 제거하세요. 이 컴퓨터에서는 보통 식별자를 다시 유도할 수 있습니다.\n자세한 내용: {1}",
    },

    quarantine: {
        cancelled: "취소했습니다 — 아무것도 변경되지 않았습니다.",
        confirm: "{0}을(를) 격리하고 빈 설정으로 새로 시작할까요?\n파일은 이름만 변경되며(*.unreadable.N), 삭제되지 않고 나중에 복원할 수 있습니다.",
        done: "*.unreadable.{0}(으)로 격리했습니다({1}개 파일). 삭제된 것은 없습니다.",
        entry_readable: "#{0} — 지금 읽을 수 있음, API {1}개: {2}",
        entry_unreadable: "#{0} — 이 컴퓨터에서는 여전히 읽을 수 없음",
        failed: "설정을 격리하지 못했습니다({0}). 아무것도 변경되지 않았습니다.",
        restore_confirm: "격리된 설정 #{0}을(를) 복원할까요? 현재 사용 중인 설정은 백업 세대로 보관됩니다.",
        restore_done: "격리된 설정 #{0}을(를) 복원했습니다(API {1}개).",
        restore_failed: "복원하지 못했습니다({0}). 아무것도 변경되지 않았습니다.",
        restore_none: "지금 이 컴퓨터에서 열 수 있는 격리된 설정이 없습니다.",
        restore_title: "격리된 설정",
    },
    ...
    errors: {
        ...
        validation: {
            ...
            auth_token_placeholder: "이것은 토큰이 없을 때 내보내기가 기록하는 자리 표시자이며, 실제 토큰이 아닙니다",
    ...
    import_export: {
        export: {
            ...
            details_skipped: "제외됨(저장된 토큰을 복호화하지 못했습니다): {0}",
    ...
    version: {
        ...
        install_command: "npm install -g @kikkimo/claude-launcher@latest을 실행하여 업데이트",
    ...
    version_check: {
        ...
        update_command: "업데이트 명령: npm install -g @kikkimo/claude-launcher@latest",
    ...
    hints: {
        ...
        model: {
            ...
            fable: "Claude Code Fable 티어에 해당",
            fable_detail: "장기 작업 및 백그라운드 작업용 플래그십 모델. 환경 변수 [ANTHROPIC_DEFAULT_FABLE_MODEL]에 해당. 공급자의 플래그십에 자동 매핑",
    ...
    config_labels: {
        model: {
            ...
            ANTHROPIC_DEFAULT_FABLE_MODEL: "Fable 모델(장기 작업)",
