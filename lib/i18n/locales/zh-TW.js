...
module.exports = {
    ...
    menu: {
        ...
        api_management: {
            ...
            quarantine_config: "隔離無法讀取的設定並重新開始",
            restore_quarantined: "還原已隔離的設定",
            back: "返回主選單"
        },
        config: {
            title: "設定管理",
    ...
    // 訊息類型
    warnings: {
        config_load_error: "API 設定檔無法讀取，未被覆寫：{0}\n可進入「第三方 API 管理」將其安全隔離（不會刪除任何檔案）後重新開始。",
        config_recovered: "API 設定檔已損壞——已自動從備份復原：{0}",
        superseded_preserved: "被頂替的那一代**沒有被刪除**，保留在 {0}。",
        snapshot_available: "磁碟上仍有一份可讀的遷移前設定快照：{0}\n它不會被自動使用。還原步驟見 README 的備份章節。",
        snapshot_unreadable: "磁碟上有一份遷移前快照，但目前金鑰開不了它：{0}\n請**不要刪除**——換到別的網路、或還原金鑰材料檔案之後，它可能重新變得可讀。",
        quarantine_available: "磁碟上仍保留著先前隔離的設定。可在「第三方 API 管理」中選擇「還原已隔離的設定」嘗試找回。",
        token_unrecoverable: "以下 API 的驗證權杖無法解密：{0}\n請重新輸入。先前的密文保留在 {1}，未刪除任何內容。",
        key_material_degraded: "機器金鑰材料：{0}",
        config_unreadable_key_material: "機器金鑰材料無法讀取，導致任何內容都解不開：{0}\n你的 API 設定檔大機率是**完好**的——請不要刪除它。應當修復或移除該金鑰材料檔案；在這台機器上身分通常可以重新推導出來。\n詳情：{1}",
    },

    quarantine: {
        cancelled: "已取消——未做任何變更。",
        confirm: "是否隔離 {0} 並以空設定重新開始？\n檔案只會被重新命名（改為 *.unreadable.N），不會刪除任何內容，之後仍可還原。",
        done: "已隔離為 *.unreadable.{0}（共 {1} 個檔案）。未刪除任何內容。",
        entry_readable: "#{0} —— 現在可讀，{1} 個 API：{2}",
        entry_unreadable: "#{0} —— 在這台機器上仍無法讀取",
        failed: "隔離失敗（{0}）。未做任何變更。",
        restore_confirm: "是否還原已隔離的設定 #{0}？目前正在使用的設定會保留為備份世代。",
        restore_done: "已還原隔離設定 #{0}（{1} 個 API）。",
        restore_failed: "還原失敗（{0}）。未做任何變更。",
        restore_none: "目前這台機器上沒有可以開啟的已隔離設定。",
        restore_title: "已隔離的設定",
    },
    ...
    errors: {
        ...
        validation: {
            ...
            auth_token_placeholder: "這是匯出時用於佔位的標記，不是真實權杖",
    ...
    import_export: {
        export: {
            ...
            details_skipped: "已略過（其儲存的權杖無法解密）：{0}",
    ...
    version: {
        ...
        install_command: "執行 npm install -g @kikkimo/claude-launcher@latest 更新",
    ...
    version_check: {
        ...
        update_command: "更新指令：npm install -g @kikkimo/claude-launcher@latest",
    ...
    hints: {
        ...
        model: {
            ...
            fable: "對應 Claude Code Fable 層級",
            fable_detail: "用於長程任務與背景任務的旗艦模型。對應環境變數 [ANTHROPIC_DEFAULT_FABLE_MODEL]。自動對應到供應商旗艦模型",
    ...
    config_labels: {
        model: {
            ...
            ANTHROPIC_DEFAULT_FABLE_MODEL: "Fable 模型（長程任務）",
