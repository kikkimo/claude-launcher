...
module.exports = {
    ...
    menu: {
        ...
        api_management: {
            ...
            quarantine_config: "Отложить нечитаемую конфигурацию и начать заново",
            restore_quarantined: "Восстановить отложенную конфигурацию",
    ...
    // Message types
    warnings: {
        config_load_error: "Файл конфигурации API не читается и НЕ был перезаписан: {0}\nОткройте управление сторонними API, чтобы безопасно отложить его (ничего не удаляется) и начать заново.",
        config_recovered: "Файл конфигурации API был повреждён — автоматически восстановлен из резервной копии: {0}",
        superseded_preserved: "Заменённое поколение НЕ удалено — оно сохранено в {0}.",
        snapshot_available: "На диске всё ещё есть читаемый снимок конфигурации до миграции: {0}\nОн никогда не используется автоматически. Порядок восстановления описан в разделе о резервных копиях в README.",
        snapshot_unreadable: "На диске есть снимок до миграции, но текущий ключ его не открывает: {0}\nНЕ удаляйте его — он может снова стать читаемым в другой сети или после восстановления файла ключевого материала.",
        quarantine_available: "Ранее отложенная конфигурация всё ещё на диске. Попробуйте «Восстановить отложенную конфигурацию» в управлении сторонними API.",
        token_unrecoverable: "Не удалось расшифровать токен аутентификации этих API: {0}\nВведите их заново. Прежний шифротекст сохранён в {1}, ничего не удалено.",
        key_material_degraded: "Ключевой материал машины: {0}",
        config_unreadable_key_material: "Ключевой материал машины не читается, поэтому ничего нельзя расшифровать: {0}\nВаш файл конфигурации API, скорее всего, ЦЕЛ — не удаляйте его. Вместо этого почините или удалите файл ключевого материала; на этой машине идентификатор обычно можно вывести заново.\nПодробности: {1}",
    },

    quarantine: {
        cancelled: "Отменено — ничего не изменено.",
        confirm: "Отложить {0} и начать с пустой конфигурации?\nФайлы только ПЕРЕИМЕНОВЫВАЮТСЯ (в *.unreadable.N) — ничего не удаляется, и позже их можно восстановить.",
        done: "Отложено как *.unreadable.{0} ({1} файл(ов)). Ничего не удалено.",
        entry_readable: "#{0} — сейчас читается, API: {1} шт.: {2}",
        entry_unreadable: "#{0} — на этой машине всё ещё не читается",
        failed: "Не удалось отложить конфигурацию ({0}). Ничего не изменено.",
        restore_confirm: "Восстановить отложенную конфигурацию #{0}? Используемая сейчас конфигурация сохранится как резервное поколение.",
        restore_done: "Отложенная конфигурация #{0} восстановлена (API: {1} шт.).",
        restore_failed: "Не удалось восстановить ({0}). Ничего не изменено.",
        restore_none: "Сейчас на этой машине нельзя открыть ни одну отложенную конфигурацию.",
        restore_title: "Отложенные конфигурации",
    },
    ...
    errors: {
        ...
        validation: {
            ...
            auth_token_placeholder: "Это заполнитель, который экспорт записывает при отсутствии токена, а не сам токен",
    ...
    import_export: {
        export: {
            ...
            details_skipped: "Пропущены (сохранённый токен не удалось расшифровать): {0}",
    ...
    version: {
        ...
        install_command: "Выполните npm install -g @kikkimo/claude-launcher@latest для обновления",
    ...
    version_check: {
        ...
        update_command: "Команда обновления: npm install -g @kikkimo/claude-launcher@latest",
    ...
    hints: {
        ...
        model: {
            ...
            fable: "Соответствует уровню Fable в Claude Code",
            fable_detail: "Флагманская модель для долгих и фоновых задач. Соответствует переменной окружения [ANTHROPIC_DEFAULT_FABLE_MODEL]. Автоматически сопоставляется с флагманом провайдера",
        ...
        runtime: {
            ...
            experimental: 'Отключает экспериментальные Beta-функции Anthropic для повышения стабильности API',
    ...
    config_labels: {
        model: {
            ...
            ANTHROPIC_DEFAULT_FABLE_MODEL: "Модель Fable (Fable)",
    ...
    config: {
        values: {
            ...
            recommended_on: 'Вкл (рекомендуется)'
        }
    }
};