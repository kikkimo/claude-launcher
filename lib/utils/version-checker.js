 ...
 * @returns {Promise<boolean>} true on success, false on refusal/failure
 *   (same boolean contract as ApiManager.saveConfig; callers that ignore
    ...
    try {
        ...
        if (fsSync.existsSync(configPath)) {
            try {
                ...
                screen.debug('[version-checker] refusing to save: config file exists but does not parse — left untouched');
                return false;
            }
        }
