...
function launchClaude(command, envVars = {}, disableAuthTokens = false) {
    ...
    // This allows Ctrl+C to be handled by Claude Code process instead
...
function launchClaudeWithApi(api, skipPermissions = false) {
    ...
    if (providerConfig && providerConfig.envVars && Object.keys(providerConfig.envVars).length > 0) {
        console.log(colors.yellow + '  ⚡ Provider Optimizations Applied' + colors.reset);

        // Display specific optimizations based on envVars
        if (providerConfig.envVars.API_TIMEOUT_MS) {
            const timeoutSec = Math.floor(parseInt(providerConfig.envVars.API_TIMEOUT_MS) / 1000);
            const timeoutMin = Math.floor(timeoutSec / 60);
            console.log(colors.gray + `    • Extended timeout: ${timeoutSec}s (${timeoutMin} minutes)` + colors.reset);
        }
