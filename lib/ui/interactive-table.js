...
async function showApiSelectionTable(apis, title, actionType = 'select', activeIndex = -1) {
    if (apis.length === 0) {
        ...
        console.log(colors.gray + 'You need to add some APIs first.' + colors.reset);
        console.log('');
    ...
    function displayApiTable() {
        ...
        if (actionType === 'switch' && activeIndex >= 0 && activeIndex < apis.length) {
            ...
            console.log(colors.gray + 'Currently Active API:' + colors.reset);
        ...
        if (actionType === 'switch' && activeIndex >= 0) {
            console.log(colors.green + '  ● = Currently active API' + colors.reset);
        }

        const actionText = actionType === 'remove' ? 'remove' : 'activate';
        console.log(colors.amber + `  Use ↑↓ to navigate, Enter to ${actionText}, ESC to return to main menu` + colors.reset);
        console.log('');
    }
...
async function confirmDeletion(api) {
    ...
    console.log('');
    console.log(colors.yellow + 'Are you sure you want to delete this API configuration?' + colors.reset);
    console.log('');
    ...
    return new Promise((resolve) => {
        rl.question(colors.red + '[?] Confirm deletion (y/N): ' + colors.reset, (answer) => {
            rl.close();
