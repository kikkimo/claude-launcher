                ...
                if (result !== undefined) {
                    ...
                    if (result && actionType === 'switch' && apiManager) {
                        ...
                        try {
                            ...
                            switchedApi = apiManager.setActiveApi(selectedIndex);
                        } catch (error) {
                            // Mutating API can refuse (concurrent instance / IO).
                            // Mirror the success path's lifecycle: wait for the
                            // key, then resolve — a bare return would leave the
                            // outer promise pending and the TUI stuck.
                            screen.render([
                                '',
                                colors.bright + colors.red + `✗ ${error.message}` + colors.reset,
                                '',
