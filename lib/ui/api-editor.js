...
async function editTextField(apiManager, api, fieldKey, fieldLabel) {
    ...
    console.clear();
    console.log('');
    console.log(colors.cyan + i18n.tSync('api.edit.current_value', currentValue) + colors.reset);
    console.log('');
    ...
    if (fieldKey === 'model') {
        ...
        if (suggested.length > 0) {
            console.log(colors.gray + '  Suggested models: ' + suggested.join(', ') + colors.reset);
            console.log('');
        }
    }
    ...
    if (input.toLowerCase() === 'exit' || input.toLowerCase() === 'quit') {
        console.log(colors.yellow + i18n.tSync('api.edit.cancelled') + colors.reset);
    ...
    try {
        ...
        // Success message
        console.log(colors.green + i18n.tSync('api.edit.success', fieldLabel) + colors.reset);
        ...
        if (fieldKey === 'baseUrl') {
            ...
            if (detected !== updated.provider) {
                ...
                console.log(colors.yellow + i18n.tSync('api.edit.url_provider_hint', detectedName, currentName) + colors.reset);
            }
        }
...
async function editProviderField(apiManager, api) {
    console.clear();
    console.log('');
    console.log(colors.cyan + i18n.tSync('api.edit.current_value', resolveProviderName(api.provider)) + colors.reset);
    console.log('');
    ...
    try {
        ...
        console.log(colors.green + i18n.tSync('api.edit.success', i18n.tSync('api.edit.field_provider')) + colors.reset);
        await waitForKey(i18n.tSync('ui.general.press_any_key_continue'));
        return updated;
    } catch (error) {
        console.log(colors.red + '❌ ' + error.message + colors.reset);
