...
async function editApiById(apiManager, { apiId, initialSection }) {
    ...
    while (true) {
        ...
        if (choice === 1) {
            ...
            await editApiEnvVarsById(apiManager, { apiId });
            currentApi = getApiById(apiManager, apiId);
        }
...
async function editModelEnvVarsMenu(apiManager, api) {
    ...
    const getLabel = (key) => MODEL_CONFIG_LABELS[key] || key;
    ...
    while (true) {
        const options = PREDEFINED_MODEL_ENV_KEYS.map((key) => {
            ...
            const label = '  ' + getLabel(key).padEnd(24);
            return colors.reset + label + displayVal + mark;
        });
        options.push(i18n.tSync('api.edit.back'));
        ...
        const hintFn = (idx) => {
            ...
            const hintKeys = {
                ANTHROPIC_DEFAULT_SONNET_MODEL: 'hints.model.sonnet',
                ANTHROPIC_DEFAULT_OPUS_MODEL: 'hints.model.opus',
                ANTHROPIC_DEFAULT_HAIKU_MODEL: 'hints.model.haiku',
                CLAUDE_CODE_SUBAGENT_MODEL: 'hints.model.subagent',
                ANTHROPIC_CUSTOM_MODEL_OPTION: 'hints.model.custom_option',
                ANTHROPIC_CUSTOM_MODEL_OPTION_NAME: 'hints.model.custom_name',
            };
            return hintKeys[key] ? i18n.tSync(hintKeys[key]) + '\n[' + key + ']' : null;
        };
...
async function editModelEnvSingleField(apiManager, api, key) {
    const { MODEL_CONFIG_LABELS } = require('../api-manager');
    const label = MODEL_CONFIG_LABELS[key] || key;
    const currentVal = api.modelEnvVars[key] || '';
...
async function editRuntimeEnvVarsMenu(apiManager, api) {
    ...
    const providerConfig = getProvider(api.provider);
    const providerDefaults = providerConfig ? providerConfig.envVars || {} : {};
    const getLabel = (key) => RUNTIME_CONFIG_LABELS[key] || key;
    ...
    while (true) {
        ...
        const options = runtimeKeys.map(key => {
            ...
            let display;
            if (val === '') {
                display = providerVal !== undefined ? providerVal : i18n.tSync('status.default');
                if (providerVal === '1' && TYPE_A_FIELDS.includes(key)) display = i18n.tSync('status.enabled');
            ...
            const mark = isOverridden ? '  ' + colors.cyan + i18n.tSync('status.overridden') + colors.reset : '';
            const label = '  ' + getLabel(key).padEnd(22);
            return colors.reset + label + display + mark;
        });
        options.push(i18n.tSync('api.edit.back'));
        menu.setOptions(options);
        const hintFn = (idx) => {
            ...
            const hintKeys = {
                API_TIMEOUT_MS: 'hints.runtime.timeout',
                CLAUDE_CODE_ATTRIBUTION_HEADER: 'hints.runtime.attribution',
                CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC: 'hints.runtime.nonessential',
                CLAUDE_CODE_EFFORT_LEVEL: 'hints.runtime.effort',
                CLAUDE_CODE_DISABLE_EXPERIMENTAL_BETAS: 'hints.runtime.experimental',
                CLAUDE_CODE_DISABLE_NONSTREAMING_FALLBACK: 'hints.runtime.nonstreaming',
            };
            return hintKeys[key] ? i18n.tSync(hintKeys[key]) + '\n[' + key + ']' : null;
        };
        const choice = await menu.navigate(null, hintFn, 'navigation.enter_to_edit');
...
async function editRuntimeSwitchField(apiManager, api, key) {
    ...
    const label = RUNTIME_CONFIG_LABELS[key] || key;
    const currentVal = api.runtimeEnvVars[key] || '';
    const isManual = (api._runtimeEnvSources || {})[key] === 'manual';
    const menu = new Menu();
    while (true) {
        let currentDisplay = i18n.tSync('action.follow_recommended');
        if (isManual && currentVal === '1') currentDisplay = i18n.tSync('status.enabled');
        if (isManual && currentVal === 'off') currentDisplay = i18n.tSync('status.disabled');
        if (isManual && currentVal === '0') currentDisplay = '0';
        screen.render([
            '', colors.cyan + label + colors.reset, '',
...
async function editRuntimeTextField(apiManager, api, key) {
    ...
    while (true) {
        screen.render([
            '', colors.cyan + label + colors.reset, '',
            i18n.tSync('status.current_value') + ': ' + (currentVal || i18n.tSync('action.follow_recommended')),
            '', i18n.tSync('action.please_choose'), '',
        ]);
        menu.setOptions([
            i18n.tSync('action.follow_recommended'),
