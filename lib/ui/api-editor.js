...
function buildFieldMenuOptions(api) {
    ...
    const customLabel = i18n.tSync('api.edit.manage_custom_env_vars').replace(/\.\.\.$/, '');
    ...
    const fields = [
        ...
        { key: 'modelEnvVars', label: i18n.tSync('api.edit.field_model_env_vars'), value: `${modelCustomCount}/${PREDEFINED_MODEL_ENV_KEYS.length}` },
        { key: 'runtimeEnvVars', label: i18n.tSync('api.edit.field_runtime_env_vars'), value: `${runtimeManualCount}/${PREDEFINED_RUNTIME_KEYS.length}` },
        { key: 'customEnvVars', label: customLabel, value: `${customCount}` },
    ];
...
async function editApiEnvVarsById(apiManager, { apiId, initialSection } = {}) {
    ...
    while (true) {
        if (initialSection === 'runtimeEnvVars') {
            await editRuntimeEnvVarsMenu(apiManager, api);
            initialSection = null;
            api = getApiById(apiManager, apiId);
            continue;
        }
        if (initialSection === 'modelEnvVars') {
            await editModelEnvVarsMenu(apiManager, api);
            initialSection = null;
            api = getApiById(apiManager, apiId);
            continue;
        }
        if (initialSection === 'customEnvVars') {
        ...
        menu.setOptions([
            buildFieldMenuOptions(api)[4],
            buildFieldMenuOptions(api)[5],
            buildFieldMenuOptions(api)[6],
