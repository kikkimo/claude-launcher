    ...
    return new Promise((resolve) => {
        if (process.stdin.isTTY) {
            ...
            const settle = (value) => {
                if (settled) return;
                settled = true;
                try { rl.close(); } catch (_) { /* already closed */ }
                scope.release();
                resolve(value);
            };
            // EOF counts as "no". Without this the question callback never fires
            // on a closed stdin and the promise hangs forever — and a confirm
            // that cannot say no is not a confirm. It also makes destructive
            // actions safe to reach from a pipe or from CI, where the answer
            // must default to refusal rather than to whatever is convenient.
            rl.on('close', () => settle(false));
            rl.question('', (answer) => settle(answer.trim().toLowerCase() === 'y'));
        }
    });
}
