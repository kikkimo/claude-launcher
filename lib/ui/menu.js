...
class Menu {
    ...
    displayHeader() {
        ...
        process.stdout.write('\n' +
            ...
            colors.orange + '  └────────────────────────────────────────┘' + colors.reset + '\n' +
            '\n' +
            colors.gray + '  Use ↑↓ arrow keys to navigate, Enter to select' + colors.reset + '\n' +
            '\n'
        );
    }
    ...
    displayMenu() {
        ...
        this.menuOptions.forEach((option, index) => {
            if (index === this.selectedIndex) {
                menuOutput += colors.orange + '  → ' + colors.black + colors.bgAmber + option + colors.reset + '\n';
            } else {
    ...
    async navigate() {
        return new Promise((resolve) => {
            ...
            if (process.stdin.isTTY) {
                ...
                console.log(colors.yellow + '  Arrow keys not available. Enter selection number (1-' + this.menuOptions.length + '): ' + colors.reset);
        ...
        function displayList() {
            ...
            items.forEach((item, index) => {
                ...
                if (index === selectedIndex) {
                    console.log(colors.orange + prefix + colors.black + colors.bgAmber + `${item.name}${activeIndicator}` + colors.reset);
                    if (item.details) {
