...
class ApiManager {
     ...
     * Any refused or failed save rolls the in-memory config back to the
     * last persisted state, so callers never keep phantom changes.
     * @returns {boolean} true when persisted
     */
    saveConfig() {
        const ok = this._saveConfigInner();
        if (!ok) this._restoreLastPersisted();
        return ok;
    }
    ...
    _saveConfigInner() {
        ...
        try {
            ...
            // If the failure hit after rotation moved main to .bak, the
            // main path is missing — restore it so the crash window never
            // survives a failed save (Codex review).
            if (!fs.existsSync(this.configFile) && fs.existsSync(bakPath)) {
                try {
                    fs.renameSync(bakPath, this.configFile);
