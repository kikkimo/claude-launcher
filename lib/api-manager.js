...
class ApiManager {
    ...
    _migrateApiEntry(api, MODEL_KEYS, RUNTIME_KEYS) {
        let changed = false;
        if (!api.modelEnvVars) { api.modelEnvVars = {}; for (const k of MODEL_KEYS) api.modelEnvVars[k] = ''; changed = true; }
        if (!api._autoModelEnvVars) {
            api._autoModelEnvVars = {};
            if (api._autoFilledModel) {
                for (const k of MODEL_KEYS) api._autoModelEnvVars[k] = api._autoFilledModel;
                api._autoModelEnvVars.smallFastModel = api.smallFastModel || api.model;
                delete api._autoFilledModel;
            } else {
                const m = api.model || '';
                for (const k of MODEL_KEYS) api._autoModelEnvVars[k] = m;
                api._autoModelEnvVars.smallFastModel = api.smallFastModel || m;
            }
            changed = true;
        }
        if (!api.runtimeEnvVars) { api.runtimeEnvVars = {}; for (const k of RUNTIME_KEYS) api.runtimeEnvVars[k] = ''; changed = true; }
        if (!api._runtimeEnvSources) { api._runtimeEnvSources = {}; for (const k of RUNTIME_KEYS) api._runtimeEnvSources[k] = 'auto'; changed = true; }
        if (!api.customEnvVars) { api.customEnvVars = {}; changed = true; }
        if (api._autoFilledModel) { delete api._autoFilledModel; changed = true; }
        return changed;
    }
