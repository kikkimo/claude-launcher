...
class ApiManager {
    constructor(configFile) {
        ...
        if (migrated) {
            ...
            // and after .bak recovery the promoted file is valid again.
            this.saveConfig();
        }
    }
     ...
     *
     * Corruption handling (issue #11): if the main file fails to decrypt,
     * parse, or pass structural validation, fall back to the .bak written
     * by the last successful save and promote it atomically over the
     * corrupt file. If there is no usable backup either, return an
     * empty config WITH loadError set — callers must treat this state
     * explicitly (first-time wizard suppressed, saves refused) instead of
     * silently persisting an empty config over the user's data.
     */
    loadConfig() {
        ...
        if (fs.existsSync(bakPath)) {
            const backup = this._tryLoadFile(bakPath);
            if (backup.config) {
                try {
                    fs.renameSync(bakPath, this.configFile);
                } catch (error) {
                    screen.debug(`[!] Could not promote .bak over corrupt file: ${error.message}`);
                }
            ...
            screen.debug(`[!] API config backup also unreadable (${backup.error})`);
        }
    ...
    saveConfig() {
        ...
        try {
            ...
            // Write to temp file and flush to disk before touching the real file.
            ...
            // Rotate current file to .bak, then atomically promote the temp file.
            if (fs.existsSync(this.configFile)) {
            ...
            // exactly what we intended to store, otherwise roll back to .bak.
            const written = fs.readFileSync(this.configFile, 'utf8');
            const verified = decrypt(written);
            if (!verified.success || verified.value !== configJson) {
                screen.debug('[!] Config write verification failed — rolling back from .bak');
                if (fs.existsSync(bakPath)) {
                    fs.renameSync(bakPath, this.configFile);
                }
                return false;
