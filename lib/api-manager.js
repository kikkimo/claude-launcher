...
class ApiManager {
    ...
    /**
     * Load configuration from encrypted file, migrate old APIs.
    ...
    /**
     * Migrate a single API entry. Independent per-field checks.
    ...
    _migrateApiEntry(api, MODEL_KEYS, RUNTIME_KEYS) {
        ...
        if (!api.modelEnvVars) {
            api.modelEnvVars = {};
            for (const k of MODEL_KEYS) api.modelEnvVars[k] = '';
            changed = true;
        }
        ...
        if (!api.runtimeEnvVars) {
            api.runtimeEnvVars = {};
            for (const k of RUNTIME_KEYS) api.runtimeEnvVars[k] = '';
            changed = true;
        }

        if (!api._runtimeEnvSources) {
            api._runtimeEnvSources = {};
            for (const k of RUNTIME_KEYS) api._runtimeEnvSources[k] = 'auto';
            changed = true;
        }
