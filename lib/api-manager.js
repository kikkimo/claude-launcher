...
class ApiManager {
    ...
    _initializeFromDisk() {
        ...
        this._enforceOwnerOnly([this.configFile, this.configFile + '.bak',
            this.configFile + '.bak2', this.configFile + '.pre-key-migration']);
        if (this.keyStale) {
    ...
    _healKeyGeneration(pending) {
        ...
        if (!this._snapshotPreHealCiphertext()) {
            // No snapshot means no safe migration — and that has to hold on the
            // ordinary save path too, or the first user edit performs exactly
            // the migration this refused (S-8).
    ...
    _snapshotPreHealCiphertext() {
        // The bytes that actually decrypted, captured by _loadResult — NOT
        // this._diskState. Those differ whenever main was corrupt and .bak was
        // the file that loaded, and snapshotting main's garbage would take the
        // only snapshot slot forever (createExclusive never overwrites).
        ...
        if (!fs.existsSync(snapshotPath)) {
            const result = createExclusive(snapshotPath, source);
