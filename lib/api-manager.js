...
class ApiManager {
    ...
    _initializeFromDisk() {
        ...
        } else if (migrated || this._keyScanMemoDirty) {
    ...
    _loadResult(loaded, recoveredFromBackup, healBlockedReason) {
        ...
        this._keyScanMemoDirty = Boolean(inner.memoChanged);
        return {
    ...
    _recoverInnerFields(config) {
        ...
        const misses = (config && config._keyScanMisses) || {};
        const fresh = {};
        ...
        for (const api of (config && config.apis) || []) {
            ...
            if (misses[digest] === fingerprint) {
                fresh[digest] = fingerprint;
                unrecoverable.push({
        ...
        const changed = JSON.stringify(fresh) !== JSON.stringify(misses);
        if (config) {
            if (Object.keys(fresh).length > 0) config._keyScanMisses = fresh;
            else delete config._keyScanMisses;
        }
        return { pending, unrecoverable, notEncrypted, memoChanged: changed };
    }
