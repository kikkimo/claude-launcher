...
class ApiManager {
    ...
    loadConfig() {
        ...
        for (const candidate of [bakPath, bak2Path]) {
            ...
            if (backup.config) {
                ...
                return this._loadResult(backup, true);
            }
     ...
     */
    _loadResult(loaded, recoveredFromBackup) {
        const inner = this._recoverInnerFields(loaded.config);
    ...
    _recoverInnerFields(config) {
        ...
        for (const api of (config && config.apis) || []) {
            ...
            if (recovered.success && this._isTrustworthyRecovery(payload, recovered.value)) {
    ...
    _healKeyGeneration(pending) {
        ...
        if (!this._snapshotPreHealCiphertext()) return false;
        ...
        for (const item of pending) {
            ...
            const verify = decrypt(enc.value);
            if (!verify.success || verify.value !== item.plaintext) {
        ...
        if (this.saveConfig()) return true;
        if (!this.saveConflict) return false;
    ...
    _snapshotPreHealCiphertext() {
        if (this._diskState === null) return true; // nothing on disk to preserve
        const snapshotPath = this.configFile + '.pre-key-migration';
        if (!fs.existsSync(snapshotPath)) {
            const result = createExclusive(snapshotPath, this._diskState);
            if (!result.created && !fs.existsSync(snapshotPath)) {
                screen.debug(`[!] Key heal aborted: could not snapshot the pre-heal config (${result.reason})`);
                return false;
            }
        }
        try {
            if (fs.readFileSync(snapshotPath, 'utf8').length === 0) {
                screen.debug('[!] Key heal aborted: pre-heal snapshot is empty');
                return false;
            }
        } catch (error) {
            screen.debug(`[!] Key heal aborted: pre-heal snapshot unreadable (${error.message})`);
            return false;
        }
        chmodOwnerOnly([snapshotPath]);
        return true;
    }
     ...
     *
     * Reconcile instead: if the disk is already healed, adopt it; otherwise
     * rebuild the CAS baseline and retry exactly once. The guard for
     * user-initiated saves is untouched.
     */
    _reconcileHealConflict(healed) {
        ...
        this._diskState = diskNow;
        this.config = JSON.parse(JSON.stringify(healed));
        this.saveConflict = false;
        this.saveOutcome = 'idle';
     ...
     * a concurrent heal.
     *
     * @returns {{config?: object, keyStale?: boolean, error?: string}}
     */
