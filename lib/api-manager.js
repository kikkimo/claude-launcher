...
class ApiManager {
    ...
    _initializeFromDisk() {
        ...
        if (this.loadError || this.config.apis.length === 0) {
            ...
            // Readable first (listSnapshots sorts that way), but an UNREADABLE
            // orphan is reported too: staying silent about it left deleting it —
            // the last copy of the user's tokens — as the only visible move.
    ...
    _snapshotPreHealCiphertext() {
        ...
        if (!fs.existsSync(slot)) {
            // The identity that can OPEN these bytes — which is the OLD one,
            // not the one we are migrating to. Naming the target identity was
            // systematically off by one generation and pointed a human doing
            // manual recovery at the one identity that cannot open the slot.
            // When we do not know it (nothing recovered this run), say so
            // rather than name the wrong one.
            const opener = this._healOpenerIdentity || null;
            const document = JSON.stringify({
                v: SNAPSHOT_VERSION,
                source: opener ? opener.source : 'unknown',
                ...
                idHint: opener
                    ? crypto.createHash('sha256').update(opener.id).digest('hex').slice(0, 12)
                    : null,
    ...
    /**
     * decryptWithRecovery, but a miss is remembered per (ciphertext, candidate
     * set) so it is paid once rather than on every launch.
     */
    _recoverCached(ciphertext) {
        const misses = this._readScanMisses();
        const digest = crypto.createHash('sha256').update(ciphertext).digest('hex').slice(0, 16);
        const fingerprint = candidateFingerprint();
        const known = Array.isArray(misses[digest]) ? misses[digest] : [];
        if (known.includes(fingerprint)) return { success: false, value: null, error: 'known miss' };
        ...
        if (!attempt.success) {
            misses[digest] = [fingerprint, ...known.filter(f => f !== fingerprint)]
                .slice(0, MAX_MEMO_FINGERPRINTS);
            this._writeScanMisses(misses);
        }
        return attempt;
    }
        ...
        return names
            ...
            .map(name => {
                ...
                if (verify && ciphertext) {
                    // Same negative cache as the field scan: a snapshot whose
                    // key is out of reach otherwise costs a full sweep on every
                    // launch, on the startup path, with nothing to show for it.
                    const attempt = this._recoverCached(ciphertext);
                    if (attempt.success) {
                        try {
        ...
        try {
            ...
            const decrypted = currentKeyOnly
                ...
                : decryptWithRecovery(encryptedData, {
                    onCandidate: (identity) => { this._healOpenerIdentity = identity; },
    ...
    _saveOrThrow() {
        if (!this.saveConfig()) {
            ...
            if (this._keyMigrationBlocked) {
                throw new Error(
                    `Cannot save: the pre-migration snapshot ` +
                    `${this._snapshotPathFor(this._healSourceBytes || '')} could not be written, and ` +
                    `re-encrypting your config without that safety copy could make existing tokens ` +
                    `unreadable. Free that path, then retry.`);
            }
            if (this.saveOutcome === 'indeterminate') {
    ...
    _saveConfigInner() {
        ...
        if (!this._acquireWriteLock(lockPath)) {
            screen.debug(`[!] Could not acquire config write lock — ${this.lockFailure}`);
            return false;
        }
        try {
    ...
    _acquireWriteLock(lockPath) {
        ...
        for (let attempt = 0; attempt < 20; attempt++) {
            try {
                ...
                lastError = error;
                // EEXIST means someone holds it and waiting may help. Anything
                // else — a read-only home, a missing directory — will not
                // improve by retrying, and reporting it as contention sends the
                // user looking for a second instance that does not exist.
                if (error.code !== 'EEXIST') break;
                _sleepSync(25);
            }
        }
        this._lockToken = null;
        this.lockFailure = lastError && lastError.code !== 'EEXIST'
            ? `cannot create the lock file (${lastError.message})`
            : 'another instance is holding the write lock';
        return false;
    }
    ...
    isFirstTimeUsage() {
        ...
        // A surviving snapshot deliberately does NOT suppress the wizard. The
        // banner is what stops an empty config looking like a fresh install,
        // and the wizard is the only route to setExportPassword() — which gates
        // import and export. Blocking it here made this release's own recovery
        // route (quarantine) permanently disable the user's ability to back up.
    ...
    exportConfigAuthenticated() {
        ...
        const exportData = {
            ...
            apis: this.config.apis.flatMap(api => {
                const { _autoFilledModel, ...safe } = api;
                // Never encrypted in the first place (legacy/plaintext entries):
                // it exports as itself. Calling that "undecryptable ciphertext"
                // would be simply untrue.
                if (typeof api.authToken === 'string' && !ENCRYPTED_PAYLOAD_RE.test(api.authToken)) {
                    return [{ ...safe, authToken: api.authToken }];
                }
                const decrypted = decrypt(api.authToken);
                if (!decrypted.success) {
                    // Skip this ONE entry rather than the whole export. A
                    // placeholder that reads like a token must never travel —
                    // it imports cleanly elsewhere and fails at request time —
                    // but refusing everything would take away the backup of
                    // every healthy API because of one that cannot be read.
                    skippedExports.push({
                        name: api.name || 'Unknown',
                        reason: `could not decrypt the stored token (${decrypted.error}); ` +
                            'enter it again for this API, or remove the API, then export once more',
                    });
                    return [];
                }
                return [{ ...safe, authToken: decrypted.value }];
            }),
            skipped: skippedExports,
    ...
    processImportData(configData) {
        ...
        configData.apis.forEach(importApi => {
            ...
            try {
                ...
                const isPlaceholder = isPlaceholderToken(importApi.authToken);
                if (isPlaceholder) {
                    // Stored token-less on purpose so url/model/env survive, but
                    // it must be REPORTED: an entry that looks configured and
                    // fails at request time is the defect this replaced.
                    warnings.push({ code: 'PLACEHOLDER_TOKEN', apiName: importApi.name || 'Unknown' });
                }
                const tokenToEncrypt = isPlaceholder ? '' : importApi.authToken;
