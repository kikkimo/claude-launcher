...
const providers = {
    ...
    deepseek: {
        ...
        modelEnvTemplate: {
            getValues(model) {
                const isFlash = model.includes('flash') || model === 'deepseek-chat';
                const mainModel = model;
                const fastModel = isFlash ? model : 'deepseek-v4-flash[1m]';
                return {
                    ANTHROPIC_CUSTOM_MODEL_OPTION: mainModel,
                    ANTHROPIC_CUSTOM_MODEL_OPTION_NAME: mainModel,
