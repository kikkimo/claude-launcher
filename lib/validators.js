...
function validateBaseUrl(url) {
    if (!url || url.trim() === '') {
        return { valid: false, error: 'Base URL is empty or missing' };
    }
...
function validateAuthToken(token) {
    if (!token || token.trim() === '') {
        return { valid: false, error: 'Auth token is empty or missing' };
    }

    if (token.length < 10) {
        return { valid: false, error: 'Auth token is too short (minimum 10 characters)' };
    }
...
function validateModel(model) {
    if (!model || model.trim() === '') {
        return { valid: false, error: 'Model name is empty or missing' };
    }
    ...
    if (!hasValidPattern && model.length < 3) {
        return { valid: false, error: 'Model name seems invalid or too short' };
    }
...
function maskApiToken(token) {
    ...
    if (token.length < 10) {
        ...
        // Show first 10, last 6, middle 10 stars: sk-a53***e2bc
        return token.substring(0, 10) + '*'.repeat(10) + token.substring(token.length - 6);
    } else {
        // Length 10-15: Show first 5, last 5, middle 10 stars
        return token.substring(0, 5) + '*'.repeat(10) + token.substring(token.length - 5);
    }
}
